...
def run():
    ...
    try:
        while True:
            ...
            # Compute and print the spiking rate for each group
            ...
            for group, timestamps in group_timestamps.items():
                ...
                print(groups[group])
                spiking_rate = len(timestamps) / (len(groups[group]) * duration)
                print ("Spiking rate for group {}: {} Hz".format(group, spiking_rate))

                group_spike_densities[group].append(spiking_rate)
