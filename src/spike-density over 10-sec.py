...
def run():
    ...
    group_spike_densities = {group: deque(maxlen=10) for group in range(1, num_groups + 1)}
    ...
    try:
        while True:
            ...
            for group, timestamps in group_timestamps.items():
                ...
                # Compute the average spike density over the last 10 seconds
                if len(group_spike_densities[group]) > 0:
                    avg_spike_density = sum(group_spike_densities[group]) / len(group_spike_densities[group])
                else:
                    avg_spike_density = 0
                print("Average spike density for group {} over last 10 seconds: {} Hz".format(group, avg_spike_density))
